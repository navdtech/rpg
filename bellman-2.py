...
def RPG(Gpi=pm.Gpi,pm=pm,p=2, rect='sa', mode ='exact', tol=0.0001):
    ...
    v = pm.v0
    for i in range(iter):   # value function approximation 
        v =RVI(v=v,p=p,rect=rect,mode=mode,tol=tol)
    Q = Qvalue(v=v,p=p,rect=rect,mode=mode,tol=tol) # Q-value from value
    FirstTerm = np.sum(np.multiply(Q,dmu[:,None])*Gpi)
    FirstTerm2 = np.multiply(Q,dmu[:,None])
    ...
    CorrectionTerm2 = coff*np.multiply(Q,du[:,None])
    return FirstTerm2 + CorrectionTerm2
...
## calculation of Q-valuesp
    for s in range(pm.S):
        for a in range(pm.A):
            Q[s,a] = pm.R[s,a] + pm.gamma*np.dot(pm.P[s,a],v)
    for s in range(pm.S):
        u[s] = np.sum(pm.pi[s]*Q[s])  - (pm.alphaS[s]+pm.betaS[s]*kappa)*np.max(pm.pi[s])
...
## calculation of Q-values
    for s in range(pm.S):
        for a in range(pm.A):
            Q[s,a] = pm.R[s,a] + pm.gamma*np.dot(pm.P[s,a],v)
    for s in range(pm.S):
        u[s] = np.sum(pm.pi[s]*Q[s])  - (pm.alphaS[s]+pm.betaS[s]*kappa)*(np.sum(pm.pi[s]**2)**(0.5))
# ##  Action step
#     Q = np.sort(Q, axis=1)        
#     for s in range(pm.S):
...
def sinf(v,pm,p='inf',tol=None, mode=None):
    ...
    for s in range(pm.S):
        u[s] = np.sum(pm.pi[s]*Q[s])  - (pm.alphaS[s]+pm.betaS[s]*kappa)
    return u
...
## calculation of Q-values
    for s in range(pm.S):
        ...
        q = p/(p-1)
        for s in range(pm.S):
            u[s] = np.sum(pm.pi[s]*Q[s])  - (pm.alphaS[s]+pm.betaS[s]*kappa)*(np.sum(pm.pi[s]**q)**(1/q))
##  Action step
    # Q = np.sort(Q, axis=1)        
    # for s in range(pm.S):
    #     sigma = pm.alphaS[s] + pm.betaS[s]*pm.gamma*kappa
